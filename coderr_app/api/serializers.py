...
class OffersSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Offer
        fields = ['id','title', 'image', 'description', 'created_at', 'updated_at', 'min_price', 'min_delivery_time', 'details']
        read_only_fields = ['min_price', 'min_delivery_time']
        write_only_fields = [ 'user']
    def _validate_details_length(self, details):
        if len(details) != 3:
            raise serializers.ValidationError({'error' : 'offer have to include exact 3 details'}) 
    ...
    def _get_current_offer_types(self, details):
        ...
        if received_types != expected_types:
            raise serializers.ValidationError({'error': 'each offer must contain the following offertypes once : basic, standard, premium'})
...
class ReviewSerializer(serializers.ModelSerializer):
    ...
    def _valdiate_double_rate(self, request):
        has_already_rated_user = Review.objects.filter(business_user=self.initial_data.get('business_user'), reviewer=request.user).exists()
        if has_already_rated_user:
            raise serializers.ValidationError({'error' : 'already rated this user'})
    ...
    def _validate_user_type(self):
        ...
        if business_user.userprofile.type == "customer":
            raise serializers.ValidationError({'error' : 'you can only rate business users'})
    ...
    def _validate_patch_fields(self, request):
        if request.method == 'PATCH':
            ...
            if request_contains_reviewer or request_contains_business_user:
                raise serializers.ValidationError({'error' : 'you can only patch rating and description'})
